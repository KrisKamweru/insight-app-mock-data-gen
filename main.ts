...
const APPS = [
  { id: 'eabank_main', name: 'EABank Mobile', countries: ['KE', 'UG', 'TZ', 'SS', 'RW'] },
  { id: 'bancaire_drc', name: 'Bancaire DRC', countries: ['DRC'] },
  { id: 'eabank_branch', name: 'EABank Branch', countries: ['KE', 'UG', 'TZ', 'SS', 'RW'], isBranchApp: true },
];

const COUNTRIES = [
  { code: 'KE', weight: 0.35, carriers: ['Safaricom', 'Airtel Kenya'], locale: 'sw-KE' },
  { code: 'UG', weight: 0.20, carriers: ['MTN Uganda', 'Airtel Uganda'], locale: 'en-UG' },
  { code: 'TZ', weight: 0.18, carriers: ['Vodacom Tanzania', 'Airtel Tanzania'], locale: 'sw-TZ' },
  { code: 'RW', weight: 0.12, carriers: ['MTN Rwanda', 'Airtel Rwanda'], locale: 'rw-RW' },
  { code: 'DRC', weight: 0.10, carriers: ['Orange DRC', 'Vodacom DRC'], locale: 'fr-DRC' },
  { code: 'SS', weight: 0.05, carriers: ['MTN South Sudan', 'Zain South Sudan'], locale: 'en-SS' },
];
...
const MOBILE_SCREENS = [
  'Login', 'Dashboard', 'AccountBalance', 'TransferMoney', 'PayBills',
  'LoanApplication', 'Settings', 'TransactionHistory', 'CustomerSupport',
  'TransferConfirmation', 'BillPayConfirmation', 'ProfileSettings'
];

const BRANCH_SCREENS = [
  'CustomerLookup', 'AccountOpening', 'TransactionProcessing', 
  'LoanProcessing', 'CustomerService', 'CashDeposit', 'CashWithdrawal',
  'AccountMaintenance', 'KYCVerification', 'DocumentScanning'
];
...
const CRASH_GROUPS = [
  { id: 'cg_network_timeout_main', type: 'NetworkTimeoutException', weight: 0.25 },
  ...
  { id: 'cg_network_timeout_drc', type: 'NetworkTimeoutException', weight: 0.10 },
...
const TRANSACTION_TYPES = [
  ...
  'loan_payment', 'mobile_money', 'card_payment'
];
...
function generateTimestamp(dayOffset: number): { timestamp: string; day: string; hour: number } {
  const now = new Date();
  ...
  const hourWeights = [1, 1, 1, 1, 1, 2, 4, 8, 12, 18, 15, 12, 8, 10, 16, 12, 8, 6, 4, 3, 2, 1, 1, 1];
...
function calculateDuration(
  ...
  else if (['UG', 'TZ'].includes(country)) multiplier *= 1.1 + Math.random() * 0.2;
  
  return Math.max(100, Math.round(logNormal(baseMs * multiplier, 0.6)));
}

function shouldCrash(platform: string, version: string, deviceTier: string, releaseChannel: string, country: string): boolean {
...
function generateSession(dayOffset: number): RawEvent[] {
  ...
  let availableApps = APPS.filter(app => app.countries.includes(country.code));
  ...
  const platform = isBranchSession && Math.random() < 0.8 ? 'web' : randomChoice<'ios'| 'android'| 'web'>(['ios', 'android', 'web']);
  ...
  const version = randomChoice(appVersions[platform] || appVersions.web || ['1.0.0']);
  
  const releaseChannel = Math.random() < 0.7 ? 'prod' : Math.random() < 0.6 ? 'pilot' : Math.random() < 0.5 ? 'uat' : 'dev';
  const networkType = Math.random() < 0.4 ? 'wifi' : Math.random() < 0.9 ? 'cellular' : 'offline';
  const carrier = networkType === 'cellular' ? randomChoice(country.carriers) : undefined;
  
  const userId = `u_${country.code.toLowerCase()}_${isBranchSession ? 'staff' : 'customer'}_${Math.floor(Math.random() * 10000)}`;
  
  const osVersion = platform === 'ios' ? `iOS ${16 + Math.floor(Math.random() * 2)}.${Math.floor(Math.random() * 6)}` :
                   platform === 'android' ? `Android ${11 + Math.floor(Math.random() * 3)}` : 'Web';
  
  const buildNumber = platform === 'ios' ? version.replace(/\./g, '') : 
                     platform === 'android' ? version.replace(/\./g, '') + '00' : version.replace(/\./g, '') + '0';
  
  // Generate banking session events
  const eventCount = Math.max(2, Math.round(CONFIG.EVENTS_PER_SESSION_AVG + (Math.random() - 0.5) * 4));
  ...
  for (let i = 0; i < eventCount; i++) {
    ...
    if (shouldCrash(platform, version, device.tier, releaseChannel, country.code)) {
      eventType = 'crash';
    } else if (Math.random() < CONFIG.ANALYTICS_RATIO / (CONFIG.ANALYTICS_RATIO + CONFIG.PERFORMANCE_RATIO)) {
      eventType = 'analytics';
    } else {
    ...
    if (eventType === 'analytics') {
      ...
      if (isBranchSession) {
        const branchEvents = ['customer_account_opened', 'transaction_processed', 'kyc_completed', 'cash_deposit', 'customer_lookup'];
        ...
        event = {
          ...
          screen,
          account_type: analyticsEvent === 'customer_account_opened' ? randomChoice(ACCOUNT_TYPES) : undefined,
          branch_code: `${country.code}_${randomChoice(['KMP', 'CBD', 'IND', 'RES'])}_${String(Math.floor(Math.random() * 999) + 1).padStart(3, '0')}`,
          value_num: analyticsEvent === 'customer_account_opened' ? 1 : undefined,
        } as RawEvent;
      } else {
        const customerEvents = ['login_success', 'balance_check', 'transaction_completed', 'bill_payment', 'login_failed'];
        analyticsEvent = hasTransaction && i === eventCount - 1 ? 'transaction_completed' : randomChoice(customerEvents);
        screen = randomChoice(screens);
        ...
        event = {
          ...
          analytics_event: analyticsEvent,
          screen,
          transaction_type: analyticsEvent === 'transaction_completed' ? randomChoice(TRANSACTION_TYPES) : undefined,
          value_num: analyticsEvent === 'transaction_completed' ? Math.round(logNormal(25000, 1.2)) : undefined,
          currency: analyticsEvent === 'transaction_completed' ? (country.code === 'KE' ? 'KES' : country.code === 'UG' ? 'UGX' : country.code === 'TZ' ? 'TZS' : 'USD') : undefined,
        } as RawEvent;
      }
    } else if (eventType === 'performance') {
      const perfTypes = ['http', 'screen', 'trace', 'app_start'];
      const perfType = randomChoice(perfTypes) as 'http' | 'screen' | 'trace' | 'app_start';
      
      const baseDurations = { http: 1200, screen: 1800, trace: 800, app_start: 3000 }; // Banking apps are slower
      ...
      if (perfType === 'http') {
        ...
        event.status_code = isError ? (Math.random() < 0.6 ? 400 : Math.random() < 0.7 ? 401 : 500) : 200;
        ...
        event.screen = randomChoice(isBranchSession ? BRANCH_SCREENS : MOBILE_SCREENS);
      } else if (perfType === 'screen') {
        event.screen = randomChoice(isBranchSession ? BRANCH_SCREENS : MOBILE_SCREENS);
        event.fps_avg = Math.max(20, Math.round(60 - (device.tier === 'low' ? 30 : device.tier === 'mid' ? 15 : 8) + Math.random() * 10));
      } else if (perfType === 'trace') {
        const traces = isBranchSession 
          ? ['kyc_verification', 'document_processing', 'account_validation', 'cash_counting']
          : ['biometric_auth', 'transaction_encryption', 'balance_calculation', 'risk_assessment'];
      ...
      const crashTypes = platform === 'android' ? ['fatal', 'nonfatal', 'anr'] : ['fatal', 'nonfatal'];
      ...
      event = {
        ...
        crash_type: isFatal ? 'fatal' : randomChoice(crashTypes.filter(t => t !== 'fatal')) as any,
...
function generateRawEvents(): RawEvent[] {
  const events: RawEvent[] = [];
  const sessionsPerDay = Math.round(CONFIG.SESSIONS_PER_DAY * (CONFIG.TOTAL_EVENTS_TARGET / 45000));
  ...
  for (let day = 0; day < CONFIG.DATE_RANGE_DAYS; day++) {
    ...
    const dailySessions = Math.round(sessionsPerDay * weekendMultiplier * (0.8 + Math.random() * 0.4));
...
function generateDailyRollups(rawEvents: RawEvent[]): DailyRollup[] {
  ...
  for (const event of rawEvents) {
    const eventGroup = event.source === 'performance' 
  ...
  return Array.from(rollups.entries()).map(([key, group]) => {
    const [day, source, platform, app_id, app_version, release_channel, country, device_tier, event_group] = key.split('|');
    ...
    if (durations.length > 0) {
      rollup.avg_duration_ms = Math.round(durations.reduce((a, b) => a + b, 0) / durations.length);
    ...
    if (source === 'crash') {
      rollup.crash_rate_per_1k_sessions = (group.events.length / group.sessions.size) * 1000;
    }
...
console.log('🏦 App distribution:', Object.entries(appStats).map(([k, v]) => `${k}: ${Math.round(v/rawEvents.length*100)}%`).join(', '));
console.log('🌍 Country distribution:', Object.entries(countryStats).map(([k, v]) => `${k}: ${Math.round(v/rawEvents.length*100)}%`).join(', '));